...
Run npm install.
Copy .env.example to .env.local (the default API URL is already configured).
Run npm run dev.
...
Finished work
 Login page using the provided DummyJSON credentials, with an error message for failed login.
 Shared Axios client that attaches the saved token and handles API errors centrally.
 Protected product routes, logout, product list and responsive desktop table/mobile cards.
 URL-backed page, page size, search, category and sort controls; debounced search and stale-result protection.
 Product details and reviews, including a not-found state for invalid product IDs.
 Add and edit forms with validation, delete confirmation, loading/empty/error states and retry.
 Setup documentation, DummyJSON limitations and suggested Git checkpoints.
Project structure and build sequence
src/types/product.ts: shared shapes for products and API pages.
src/lib/auth.ts and src/lib/http.ts: save the token and attach it with a shared Axios request interceptor; normalize errors and redirect on 401.
src/lib/api/: endpoint functions keep HTTP details outside components.
src/app/layout.tsx, globals.css, and components/: common shell, responsive styling and small reusable pieces.
/login: submits the provided credentials and prevents duplicate submits.
/products: URL parameters hold page, size, q, category and sort; API results load with a debounce and stale requests are ignored. Pagination and sorting are implemented by this project.
/products/[id]: product details and reviews; invalid IDs and missing products show a not-found message.
/products/new and /products/[id]/edit: validated forms; delete asks for confirmation.
...
chore: scaffold Next.js dashboard
feat: add shared Axios client and login
feat: build product list and URL controls
feat: add product details and reviews
feat: add product create edit and delete flows
docs: document setup and API limitations
Short project note
